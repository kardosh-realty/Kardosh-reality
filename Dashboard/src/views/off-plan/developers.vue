  ...
  <div class="container-fluid relative px-3 dash-page">
    <div class="layout-specing">
      <div class="md:flex justify-between items-center">
        <PageHeader title="Developers" crumb="All developers from the website — hiding a developer also hides all of their projects" />
        <span class="text-slate-400 text-sm mt-2 md:mt-0">
          {{ visibleCount }} shown · {{ developers.length - visibleCount }} hidden
        </span>
      </div>
      ...
      <p v-else-if="!filtered.length" class="mt-6 text-slate-400">No developers match.</p>
      ...
      <div v-else class="grid xl:grid-cols-4 md:grid-cols-2 grid-cols-1 mt-6 gap-6">
        <div
          ...
          :class="dev.hidden ? 'opacity-60' : ''"
        >
          <div class="p-5">
            <div class="flex items-center justify-between">
              <span class="size-10 flex items-center justify-center rounded-md bg-primary/10 text-primary overflow-hidden">
                <img v-if="dev.image" :src="dev.image" class="size-full object-cover" alt="" />
                <i v-else class="ri-community-line text-xl" />
              </span>
              <span
                v-if="dev.hidden"
            ...
            <h6 class="text-lg font-semibold mt-3">{{ dev.name }}</h6>
            <p class="text-slate-400 text-sm">{{ dev.projectCount }} projects</p>
            ...
            >
              <i :class="dev.hidden ? 'ri-eye-line' : 'ri-eye-off-line'" />
              {{ dev.hidden ? 'Show developer' : 'Hide developer' }}
            </button>
          </div>
        </div>
      </div>
...
const { developers, listLoading: loading, error, source, dbError, loadOffPlan, toggleDeveloper } = useOffPlan()

const query = ref('')
const visibleCount = computed(() => developers.value.filter((d) => !d.hidden).length)
const filtered = computed(() => {
  const q = query.value.trim().toLowerCase()
  if (!q) return developers.value
